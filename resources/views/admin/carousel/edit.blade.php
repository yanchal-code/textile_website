<div class="container">
    <form id="carouselCreatForm" class="needs-validation custom-form" novalidate method="POST"
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="heighlight" name="heighlight" value="{{ $carousel->heighlight }}" required>
        </div>

        <div class="mb-3">
            <label for="image_path" class="form-label">Image</label>
            <input type="file" class="form-control img_input" id="image_path" name="image_path">

            <span>Current</span>

            <img src="{{ asset($carousel->image_path) }}" alt="img" class="img-fluid" style="max-height:100px;">
        </div>


        <div class="mb-3">
            <label for="btn_text" class="form-label">Btn Text</label>
            <input value="{{ $carousel->btn_text }}" required type="btn_text" class="form-control" id="btn_text"
                name="btn_text">
        </div>
        ...
        <div class="mb-3">
            ...
            <select type="text" name="status" id="status" class="form-select">
                <option @if ($carousel->status == 1) selected @endif value="1">Active
                </option>
                <option @if ($carousel->status == 0) selected @endif value="0">Inactive
                </option>
            </select>
        </div>
...
<script>
    $('#carouselCreatForm').submit(function(e) {
        ...
        if (form[0].checkValidity() === true) {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('carousel.update', $carousel) }}",
                data: formData,
                headers: {
