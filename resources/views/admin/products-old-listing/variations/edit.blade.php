...
@section('content')
    ...
    <div class="container py-3">
        <div class="h4 my-3">{{ $variation->product->name }}</div>
        <form id="product-form" action="{{ route('variations.edit.store', $variation->id) }}" class="needs-validation"
            novalidate method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">

                <div class="mb-3 col-md-6">
                    <label for="color" class="form-label">{{ $variation->product->leafCategory->v1st }} <span
                            class="text-danger">*</span> </label>
                    <input type="text" value="{{ $variation->color }}" class="form-control" id="color" name="color"
                        required>
                </div>

                <div class="mb-3 col-md-6">
                    <label for="size" class="form-label">{{ $variation->product->leafCategory->v2nd }} <span
                            class="text-danger">*</span></label>
                    <input type="text" value="{{ $variation->size }}" class="form-control" id="size" name="size"
                        required>
                </div>
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input type="text" value="{{ $variation->sku }}" class="form-control" id="sku" name="sku"
                        required>
                </div>

                <div class="mb-3 col-md-6">
                    <label for="price" class="form-label">Price <span class="text-danger">*</span></label>
                    <input type="text" value="{{ $variation->price }}" class="form-control" min="0.00" id="price"
                        name="price" required>
                </div>

                <div class="mb-3 col-md-6">
                    <label for="price" class="form-label">C Price</label>
                    <input type="text" value="{{ $variation->c_price }}" class="form-control" min="0.00"
                        id="price" name="c_price">
                </div>

                <div class="mb-3 col-md-6">
                    <label for="quantity" class="form-label">Quantity <span class="text-danger">*</span></label>
                    <input type="text" value="{{ $variation->quantity }}" class="form-control" id="quantity"
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <select required type="text" name="status" id="status" class="form-select">
                            <option {{ $variation->status == 1 ? 'selected' : '' }} value="1">Active</option>
                            <option {{ $variation->status == 0 ? 'selected' : '' }} value="0">Inactive</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <div>
                        <div class="row px-2 justify-content-start" id="productImageDivMain">
                            ...
                            @php
                                $totalImages = $variation->images->count();
                            @endphp

                            @if ($totalImages > 0)
                                @foreach ($variation->images as $productImage)
            ...
            <button id="product-create-button" type="submit" class="btn btn-success my-3">Submit Variation</button>
        </form>
    </div>
    ...
    <script>
        ...
        const dropzone = $("#image").dropzone({
            ...
            url: "{{ route('tempImages.create', ['productId' => $variation->product->id, 'variationId' => $variation->id]) }}",
            maxFiles: 10,
            paramName: 'image',
            addRemoveLinks: true,
                        ...
                        success: function(response) {
                            ...
                            if (response.status === false) {
                                ...
                                window.location.href =
                                    "{{ route('products.variations', $variation->product->id) }}"
